...
vd_reg_month <- vd %>% 
  ...
  rename(nuts3 = kraj_nuts_kod, nuts3_name = kraj_nazev) %>% 
  # there is a row for each day, so let's group by month
  group_by(month, vakcina, nuts3, nuts3_name) %>%
  # and summarise for each of those months
  summarise(total_doses = sum(celkem_davek)) %>% 
  ungroup() %>% 
  arrange(month) %>% 
  group_by(vakcina, nuts3, nuts3_name) %>% 
  mutate(cum_doses = cumsum(total_doses))
```
...
ggplot(data = vd_reg_month, #BREAK
       mapping = aes(x = month, y = cum_doses) #BREAK2
) + #BREAK
  geom_col( #BREAK3
    aes(fill = vakcina) #BREAK4
  ) + #BREAK3
...
Working with Czech public data? Check out packages: {CzechData}, {RCzechia}, {czso}
